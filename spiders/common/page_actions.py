...
from spiders.common.constantFields import SMALL_TIMEOUT, LARGE_TIMEOUT, EXTREME_TIMEOUT
...
def wait_for_element_list_visible(driver, selectorList, by=By.CSS_SELECTOR,
                                  timeout=LARGE_TIMEOUT):
    """
    Searches for the specified element by the given selector. Returns the
    element object if the element is present and visible on the page.
    ...
    if not element and by != By.LINK_TEXT:
        raise ElementNotVisibleException(
            "Element1 {%s} was not visible after %s seconds!" % (
                selector, timeout))
    if not element and by == By.LINK_TEXT:
        raise ElementNotVisibleException(
            "Link text {%s} was not visible after %s seconds!" % (
...
def wait_for_ready_state_complete(driver, timeout=EXTREME_TIMEOUT):
    """
...
def wait_for_and_accept_alert(driver, timeout=LARGE_TIMEOUT):
    """
...
def wait_for_and_dismiss_alert(driver, timeout=LARGE_TIMEOUT):
    """
...
def wait_for_and_switch_to_alert(driver, timeout=LARGE_TIMEOUT):
    """
...
def activate_jquery(driver):
    ...
    driver.execute_script(
        ...
        '''script.src = "https://cdn.bootcss.com/jquery/3.1.0/jquery.min.js"; '''
